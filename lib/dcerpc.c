                             ...
                             uint8_t *out, int cap)
{
        ...
        /* NDR stub, encoded exactly like the client path (top-level coder that
         * handles the conformance run + deferred pointers itself). */
        pdu->top_level = 1;
        pdu->cur_ptr = 0;
        pdu->max_ptr = 0;
        if (rep_coder(dce, pdu, &iov, &offset, rep)) {
                goto fail;
        }
